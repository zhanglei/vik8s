...
func AutoLabelNodes(nodes ssh.Nodes, labels map[string]string, nodeNames ...string) {
	labeledNodes := SearchLabelNode(nodes.Master(), labels)

	//check label
	for _, labeledNode := range labeledNodes {
		utils.Assert(utils.Search(nodeNames, labeledNode) >= 0,
			color.FgRed.Render("node %s include label %s "), labeledNode, utils.Join(labels, ",", "="))
	}

	//add label
	for _, selectNode := range nodeNames {
		if utils.Search(labeledNodes, selectNode) == -1 {
			AddNodeLabel(nodes.Master(), labels, selectNode)
		}
	}
}
