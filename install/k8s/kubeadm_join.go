...
func JoinControl(node *ssh.Node) {
	...
	if exists, _ := Config.ExistsNode(node.Host); exists {
		color.Red.Printf("%s already in the cluster\n", node.Host)
		return
	}
	...
	setNodeHosts(node)
	install.InstallNTPServices(node, Config.Timezone, master.Host)
	setApiServerHosts(node)
	setIpvsadmApiServer(master, node)
	makeCerts(node)

	joinCmd := getJoinCmd(master)
	control := fmt.Sprintf("%s --control-plane --apiserver-advertise-address %s", joinCmd, node.Host)
...
func JoinWorker(node *ssh.Node) {
	...
	joinCmd := getJoinCmd(master)
	cmd := fmt.Sprintf("%s --apiserver-advertise-address=%s", joinCmd, node.Host)
