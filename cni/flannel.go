...
func (f *flannel) Flags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&f.repo, flags(f, "repo"), "", repo.QuayIODesc())
}
...
func (f *flannel) Clean(node *ssh.Node) {
	_, _ = node.Cmd("ifconfig | grep flannel.1 > /dev/null && ifconfig flannel.1 down")
	_, _ = node.Cmd("ip link show | grep flannel.1 && ip link delete flannel.1 ")
}
