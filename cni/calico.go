...
func (f *calico) Apply(node *ssh.Node) {
	...
	if f.etcd.Enable {
		if len(f.etcd.Endpoints) == 0 {
			...
			if k8s.Config.ETCD.External {
				...
				f.applyVik8sETCDServer(node, vip)
	...
	tools.MustScpAndApplyAssert(node, local, data)
...
func (f *calico) Clean(node *ssh.Node) {
	_, _ = node.Cmd("ifconfig | grep tunl0 > /dev/null && ifconfig tunl0 down")
	_, _ = node.Cmd("ip link show | grep tunl0@NONE && ip link delete tunl0@NONE ")
}
