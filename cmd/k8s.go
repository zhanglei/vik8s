...
func init() {
	tenYear := time.Now().AddDate(4, 0, 0).Sub(time.Now())
	...
	initCmd.Flags().StringVar(&k8s.Config.Docker.Registry, "docker-registry", "", "Customize docker registry, ignore it when set --docker-daemon")
...
var resetCmd = &cobra.Command{
	...
	PreRun: k8s.Config.LoadCmd,
	Run: func(cmd *cobra.Command, args []string) {
		masters, _ := cmd.Flags().GetStringSlice("master")
		nodes, _ := cmd.Flags().GetStringSlice("node")

		if len(masters) == 0 && len(nodes) == 0 {
			masters = utils.Reverse(k8s.Config.Masters)
			nodes = k8s.Config.Nodes
		}

		for _, node := range append(nodes, masters...) {
			k8s.ResetNode(hosts.Get(node))
		}
		fmt.Println("-=-=-=- SUCCESS -=-=-=-")
	},
}
...
func init() {
	resetCmd.Flags().StringSliceP("master", "m", []string{}, "")
	resetCmd.Flags().StringSliceP("node", "n", []string{}, "")
}

var configCmd = &cobra.Command{
	Use: "config", Short: "Show yaml file used by vik8s deployment cluster",
	Args: cobra.ExactValidArgs(1), ValidArgs: append([]string{"all", "*"}, yamls.AssetNames()...),
